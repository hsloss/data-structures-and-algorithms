...
const count = (target, input) => {
  if(input === []){
    ...
    let filterEmptyArrays = input.filter(x => x.length !== 0)
    let countOccurences = filterEmptyArrays.reduce(function(prev, curr) {
      return (prev.concat(curr))
    })
    let count = 0
    countOccurences.forEach(function(element) {
      if(element === target){
        count++
      }
    })
...
const totalSum = (input) => {
  let concatArrays = input.reduce(function(prev, curr) {
    return (prev.concat(curr))
  })
  let sumOfValues = concatArrays.reduce(function(prev, curr) {
    return prev + curr
  })
  return sumOfValues
}
